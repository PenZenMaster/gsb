...
const docs = require('../ads-api/assets/generated-docs.json');
...
(async () => {
  const outputPath = path.join(__dirname, '../output/upgraded-docs.json');
  const results = [];

  for (const doc of docs) {
    console.log(`🔁 Upgrading: ${doc.pageTitle}`);
    const upgraded = await upgradeContent(doc);
    results.push(upgraded);
    console.log(`✅ Upgraded: ${doc.pageTitle}`);
  }

  fs.writeFileSync(outputPath, JSON.stringify(results, null, 2));
  console.log(`🧠 All upgraded content saved to ${outputPath}`);
})();
